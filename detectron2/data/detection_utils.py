...
def transform_rotated_instance_annotations(
    ...
    Returns:
        dict:
            the same input dict with fields "bbox", "segmentation", "keypoints"
            transformed according to `transforms`.
            The "bbox_mode" field will be set to XYWHA_ABS.
    """
    # clip transformed bbox to image size
...
def annotations_to_instances_rotated(annos, image_size, mask_format):
    """
    Create an :class:`Instances` object used by the models,
    from instance annotations in the dataset dict.
    ...
    if mask_format == "polygon":
        ...
        raise ValueError("polygon is the only supported mask format. Current mask format:", mask_format)
    return target
